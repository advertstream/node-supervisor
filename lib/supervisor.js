...
function run (args) {
  ...
  try {
    ...
    [ "SIGTERM", "SIGINT", "SIGHUP", "SIGQUIT" ].forEach( function(receivedSignal) {
      process.on(receivedSignal, function () {
        var child = exports.child;

        if (child) {
          if (signal === 'SIGKILL') {
            log("Received "+receivedSignal+" Sending SIGKILL to child...");
            process.kill(-child.pid, 'SIGKILL');
          } else {
            log("Received "+receivedSignal+" Sending "+receivedSignal+" to child...");
            process.kill(child.pid, receivedSignal);
          }
        }
        process.exit();
      });
...
function startProgram (prog, exec) {
  ...
  crash_queued = false;
  var child = exports.child = spawn(exec, prog, {stdio: 'inherit', detached: signal === 'SIGKILL', cwd: cwd});
  if (child.stdout) {
...
function crash () {
  ...
  setTimeout(function() {
    if (child) {
      log("crashing child ("+signal+")");
      process.kill(signal === 'SIGKILL' ? -child.pid : child.pid, signal);
    } else {
